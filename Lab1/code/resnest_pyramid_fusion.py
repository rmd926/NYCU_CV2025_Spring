...
import timm  # Used for building the resnest101e model
...
# Custom ImageFolder (sorted by numeric order)
...
# Label Smoothing CrossEntropy Loss Function
...
# Pyramid Feature Fusion + SE Module + ResNeSt101e
...
class MultiScaleResNeStSE(nn.Module):
    ...
    def forward(self, x):
        ...
        # Upsample x3, x4 to the size of x2
        B, c2, h2, w2 = x2.shape
        ...
        # Concatenate: channels = 512 + 1024 + 2048 = 3584
...
def train_one_epoch(model, dataloader, criterion, optimizer, epoch_idx,
                    total_epochs, cutmix_alpha=CUTMIX_ALPHA,
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        if (val_acc > best_val_acc) or (
            ...
            filename = (f"ResNeSt101e_MultiScaleSE_best_loss_{val_loss:.4f}_acc_{val_acc:.4f}.pt")
